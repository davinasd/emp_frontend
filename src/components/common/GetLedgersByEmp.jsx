...
const GetLedgersByEmp = ({ open, setOpen }) => {
    const onBrandSearch = () => { }
    ...
    useEffect(() => {
        try {
            axios.get(`${import.meta.env.VITE_API_BASE}/api/admin/getLeadgerbyEmployeeId/${selectedBrand}`)
                .then((res) => {
    ...
    return (
        <Drawer
            ...
            height="100vh"
            title={
                <h1 className="text-[24px] flex gap-2 items-center text-gray-700 max-w-[700px]">Ledger By Employee</h1>
            }
            extra={
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            ...
                            {clients.map((client, idx) => (
                                <option key={`${idx}-${client.employee_id}`} value={client.employee_id}>
                                    {client.name}
                                </option>
                            ))}
                        </Select>
...
export default GetLedgersByEmp